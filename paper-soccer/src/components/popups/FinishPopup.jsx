...
function FinishPopup({ winner="Player", reason = "" }) {
    ...
    return (
        <div className="absolute flex items-center justify-center w-full h-full bg-black/50">
            <div className={`relative w-[40rem] aspect-video bg-[url('/images/paper-light.png')] dark:bg-[url('/images/paper-dark.png')] flex flex-col space-y-10 items-center justify-center rounded-xl px-8${opacity ? " opacity-50" : ""}`}>
                <div className="flex flex-col items-center justify-center space-y-6">
                    ...
                    <h2 className="text-2xl font-bold text-center font-strokedim dark:font-normal">{reason}</h2>
                    <Link to="/" className="absolute text-xl font-bold bottom-10 font-strokedim dark:font-normal"><i>- Return to main menu -</i></Link>
                </div>
            </div>
        </div>
    )
